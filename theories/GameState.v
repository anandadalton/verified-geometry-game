...
Definition game_engine (s : State) (e : Event) : State :=
  ...
  | UserSelecting selection, CardClick card =>
    ...
    | NewSel new_selection => {| deck := s.(deck); board := s.(board); phase := UserSelecting new_selection |}
    | TriggerCheck (mkTriple c1 c2 c3 as t) =>
      if algebraic_validb_points c1 c2 c3
      then {| deck := s.(deck); board := s.(board); phase := AnimatingMatch t |}
      else {| deck := s.(deck); board := s.(board); phase := AnimatingFail t |}
    end
  | UserSelecting _, AnimationOver => s (* AnimationOver should not happen. *)
  | AnimatingMatch _, CardClick _ => s (* Ignore user input during animations. *)
  | AnimatingMatch selection, AnimationOver =>
    let board_minus_match := remove_triple selection s.(board) in
      let '(deck', board', dealt) := deal s.(deck) board_minus_match [] in
        match dealt with
        | [] => if referee board'
                then {| deck := deck'; board := board'; phase := UserSelecting Sel0 |}
                else {| deck := deck'; board := board'; phase := GameOver |}
        | _ => {| deck := deck'; board := board'; phase := AnimatingDeal dealt |}
        end
  | AnimatingFail _, CardClick _ => s (* Ignore user input during animations. *)
  | AnimatingFail selection, AnimationOver =>
      {| deck := s.(deck); board := s.(board); phase := UserSelecting Sel0 |}
  | AnimatingDeal _, CardClick _ => s (* Ignore user input during animations. *)
  | AnimatingDeal selection, AnimationOver =>
      if referee s.(board)
      then {| deck := s.(deck); board := s.(board); phase := UserSelecting Sel0 |}
      else {| deck := s.(deck); board := s.(board); phase := GameOver |}
  | GameOver, _ => s
  end.